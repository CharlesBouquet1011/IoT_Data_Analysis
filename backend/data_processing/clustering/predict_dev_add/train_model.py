...
def train(json_file_or_dir=None, output_dir=None, year=None, month=None, categories=None):
    """Entraîner le modèle de prédiction Dev_Add.
    ...
    # On garde les paquets avec Dev_Add connu (tous types de paquets)
    df = df[(df["Dev_Add"].notna()) & (df["Dev_Add"] != "")].reset_index(drop=True)
    print(f"Filtered to {len(df)} packets with Dev_Add")
    ...
    print(f"   - Total unique Dev_Add: {len(device_counts)}")
    print(f"   - Dev_Add with >=10 packets: {(device_counts >= 10).sum()}")
    print(f"   - Dev_Add with 5-9 packets: {((device_counts >= 5) & (device_counts < 10)).sum()}")
    print(f"   - Dev_Add with 2-4 packets: {((device_counts >= 2) & (device_counts < 5)).sum()}")
    print(f"   - Dev_Add with only 1 packet: {(device_counts == 1).sum()}")
    ...
    if len(y_sup) > 0:
        ...
        if min_samples_per_class >= 3:
            ...
            print(f"Training classifier on {X_train.shape[0]} samples / {len(np.unique(y_enc))} unique Dev_Add (ALL devices included, with cv=3 calibration)")
            ...
            print(f"Training classifier on {X_train.shape[0]} samples / {len(np.unique(y_enc))} unique Dev_Add (ALL devices included, with cv=2 calibration)")
            classifier.fit(X_train, y_train)
        else:
            print(f"Training base classifier on {X_train.shape[0]} samples / {len(np.unique(y_enc))} unique Dev_Add (ALL devices included)")
    ...
    for col in num_cols:
        try:
            med = df.groupby("Dev_Add")[col].median()
            iqr = df.groupby("Dev_Add")[col].quantile(0.75) - df.groupby("Dev_Add")[col].quantile(0.25)
            med = med.rename(f"{col}_median")
            iqr = iqr.rename(f"{col}_iqr")
            agg = agg.merge(med.reset_index(), on="Dev_Add", how="left")
            agg = agg.merge(iqr.reset_index(), on="Dev_Add", how="left")
        except Exception:
            pass

    try:
        pkt_cnt = df.groupby("Dev_Add").size().rename("packet_count")
        agg = agg.merge(pkt_cnt.reset_index(), on="Dev_Add", how="left")
    except Exception:
        pass
