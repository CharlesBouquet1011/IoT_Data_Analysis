...
# Le modèle a été entraîné sur tous les types de paquets avec Dev_Add
# Il peut maintenant prédire le Dev_Add d'un dispositif à partir des caractéristiques radio
...
def predict_device(packet: dict, top_k: int = 10):
    """
    Prédit le Dev_Add associé aux caractéristiques radio d'un paquet.
    
    Le modèle a été entraîné sur tous les types de paquets qui contiennent le Dev_Add.
    Il utilise les caractéristiques radio (SF, RSSI, LSNR, freq, etc.) pour prédire
    l'adresse du dispositif.
    
    Args:
        packet: Dictionnaire contenant les caractéristiques radio (SF, RSSI, LSNR, freq, etc.)
        top_k: Nombre de candidats Dev_Add à retourner
    
    Returns:
        Dictionnaire avec la prédiction du Dev_Add et les candidats possibles
    """
    models = load_model()
...
if __name__ == "__main__":
    ...
    print("Prédiction du Dev_Add:")
    print(f"  Dev_Add prédit: {result.get('prediction', 'N/A')}")
    print(f"  Confiance: {result.get('confidence', 0):.2%}")
    if 'candidates' in result:
        print(f"\n  Top candidats Dev_Add:")
